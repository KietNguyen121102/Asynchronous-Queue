...
public class SlotSimulation {
    ...
    public static void slotSimulation(int onlineBaristas, int physicalBaristas, double processingTime, int ArrivalRate, double alpha, ArrayList<Integer> decisions) {
        // every minute in an 8-hour working day
        int numCustomer = 0;
        int numCustomerServed = 0;

        ArrayList<Double> barista1 = new ArrayList<Double>();
        ArrayList<Double> barista2 = new ArrayList<Double>();
        ...
        for (int time = 1; time <= 4800; time = time + 1) {
            ...
            if (customer.getArrivalTime() != 0 && numCustomer <= decisions.size()) {
        
                
                int decision = decisions.get(numCustomer-1);
            ...
            for (int num = 0; num < onlineBaristas; num++) {
                if ((time >= onlineBarista[num]) && ((physicalLine.size() != 0) || (onlineQueue.size() != 0))) {
                    if ((onlineQueue.size() != 0)) {
                        ...
                        onlineQueue.poll();
                        numCustomerServed++;
                        barista1.add(servingCustomer.getArrivalTime());
            ...
            for (int num = 0; num < physicalBaristas; num++) {
                if ((time >= physicalBarista[num]) && ((physicalLine.size() != 0) || (onlineQueue.size() != 0))) {
                    if ((physicalLine.size() != 0)) {
                        ...
                        physicalLine.poll();
                        numCustomerServed++;
                        barista2.add(servingCustomer2.getArrivalTime());
        ...
        for(double i = 1; i <= decisions.size(); i = i + 1.0){
        ...
        System.out.println(barista1);
        System.out.println(barista2);
