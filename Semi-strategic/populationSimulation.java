...
public class populationSimulation {
    ...
    public static double populationSimulation(int onlineBaristas, int physicalBaristas, double processingTime, int ArrivalRate, double alpha, double physicalPopulation, double onlinePopulation, int totalCustomer){
        ...
        for (int time = 1; time <= 10000; time = time + 1) {
            ...
            if (customer.getArrivalTime() != 0) {
                ...
                if (i < physicalPopulation && numCustomer < totalCustomer) {
                    physicalLine.add(customer);
                    // System.out.println("Customer " + customer.getArrivalTime() + " chooses physical line");

                } else if(i >= physicalPopulation && i <= (onlinePopulation + physicalPopulation) && numCustomer < totalCustomer) {
